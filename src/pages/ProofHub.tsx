...
import { ArrowRight, Award, BadgeCheck, Check, ChevronRight, Clock3, Code2, FileUp, Link2, Medal, Search, ShieldCheck, Sparkles, Target, Trophy, Users, X } from 'lucide-react';
...
function WorkModal({ item, certificate, onClose, onSubmit }: {
  ...
  const canSubmit = summary.trim().length >= 40 && workUrl.trim().length >= 8;
  ...
  return <div className="fixed inset-0 z-[80] flex items-center justify-center bg-[#071426]/70 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="proof-dialog-title">
    <motion.div initial={{ opacity: 0, y: 18, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-[2rem] bg-white shadow-[0_35px_100px_rgba(0,0,0,0.35)]">
      {certificate ? <div className="p-6 sm:p-9">
        <div className="flex items-start justify-between"><div className="flex items-center gap-4"><EmployerLogo item={item} large /><div><p className="text-xs font-extrabold uppercase tracking-[0.14em] text-[#155eef]">JobX-issued credential</p><p className="mt-1 text-sm font-bold text-slate-500">{certificate.employer}</p></div></div><button onClick={onClose} aria-label="Close certificate" className="rounded-full bg-slate-100 p-2 text-slate-500"><X className="h-5 w-5" /></button></div>
        ...
        <form onSubmit={(event) => { event.preventDefault(); if (canSubmit) onSubmit(summary); }} className="p-6 sm:p-8">
...
export default function ProofHub() {
  ...
  const items = useMemo(() => PROOF_OPPORTUNITIES.filter((item) => {
    ...
    return (filter === 'All' || item.difficulty === filter) && (!q || `${item.title} ${item.employer} ${item.skill}`.toLowerCase().includes(q));
  }), [filter, query]);
    ...
    <div className="w-full pb-10">
      ...
      <section className="relative overflow-hidden rounded-[2rem] bg-[#12213a] px-5 py-7 text-white sm:px-8 lg:px-10 lg:py-10">
        ...
        <div className="relative grid gap-8 lg:grid-cols-[1fr_22rem] lg:items-center">
          <div>
            <span className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-3 py-1.5 text-xs font-extrabold uppercase tracking-[0.12em] text-blue-200"><Target className="h-4 w-4" /> Proof, not purchases</span>
            <h1 className="mt-5 max-w-3xl text-3xl font-black tracking-[-0.05em] sm:text-4xl">Build proof through JobX projects.</h1>
            <p className="mt-3 max-w-2xl text-sm font-medium leading-6 text-slate-300 sm:text-base">Solve projects created by JobX and contribute to open challenges. Every finished outcome strengthens your Career Passport.</p>
            <div className="mt-6 flex flex-wrap gap-3">
              <Link to="/projects" className="product-focus rounded-xl bg-white px-4 py-2.5 text-sm font-extrabold text-[#12213a]">JobX projects</Link>
      ...
      <section className="mt-8">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div><p className="text-xs font-extrabold uppercase tracking-[0.14em] text-[#155eef]">Project board</p><h2 className="mt-1 text-2xl font-extrabold tracking-[-0.04em] text-slate-950 dark:text-white">Projects & challenges by JobX</h2></div>
