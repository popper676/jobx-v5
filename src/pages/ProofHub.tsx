  ...
  return <div className="fixed inset-0 z-[80] flex items-center justify-center bg-[#071426]/70 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="proof-dialog-title">
    <motion.div initial={{ opacity: 0, y: 18, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-[2rem] bg-white shadow-[0_35px_100px_rgba(0,0,0,0.35)]">
      {certificate ? <div className="p-6 sm:p-9">
        <div className="flex items-start justify-between"><div className="flex items-center gap-4"><EmployerLogo item={item} large /><div><p className="text-xs font-extrabold uppercase tracking-[0.14em] text-[#155eef]">Employer-issued credential</p><p className="mt-1 text-sm font-bold text-slate-500">{certificate.employer}</p></div></div><button onClick={onClose} aria-label="Close certificate" className="rounded-full bg-slate-100 p-2 text-slate-500"><X className="h-5 w-5" /></button></div>
        <div className="mt-8 rounded-[1.5rem] border-2 border-[#155eef]/15 bg-[radial-gradient(circle_at_top_right,rgba(21,94,239,0.12),transparent_40%),linear-gradient(135deg,#fff,#f8fbff)] p-6 text-center sm:p-9">
          ...
          <p className="mx-auto mt-3 max-w-lg text-sm leading-6 text-slate-500">Awarded to <strong className="text-slate-800">Alex Rivera</strong> for completing an employer-reviewed {item.type} demonstrating verified capability in <strong className="text-slate-800">{certificate.skill}</strong>.</p>
        ...
        <form onSubmit={(event) => { event.preventDefault(); if (canSubmit) onSubmit(isTest ? answer : summary); }} className="p-6 sm:p-8">
          ...
          <div className="mt-7 flex items-center justify-between gap-4 border-t border-slate-100 pt-5"><p className="max-w-sm text-xs leading-5 text-slate-500"><ShieldCheck className="mr-1 inline h-3.5 w-3.5 text-[#155eef]" />Submission is reviewed under {item.employer}'s verified employer identity.</p><button type="submit" disabled={!canSubmit} className="inline-flex shrink-0 items-center gap-2 rounded-xl bg-[#155eef] px-5 py-3 text-sm font-extrabold text-white disabled:cursor-not-allowed disabled:opacity-40">Submit for review <ArrowRight className="h-4 w-4" /></button></div>
        </form>
      </>}
    </motion.div>
  </div>;
    ...
    <div className="w-full pb-10">
      ...
      <section className="relative overflow-hidden rounded-[2rem] bg-[#12213a] px-5 py-7 text-white sm:px-8 lg:px-10 lg:py-10">
        ...
        <div className="relative grid gap-8 lg:grid-cols-[1fr_22rem] lg:items-center">
          <div>
            ...
            <h1 className="mt-5 max-w-3xl text-3xl font-black tracking-[-0.05em] sm:text-4xl">{isTests ? 'Skill tests that show how you think.' : 'Build proof through real missions.'}</h1>
            <p className="mt-3 max-w-2xl text-sm font-medium leading-6 text-slate-300 sm:text-base">{isTests ? 'Complete practical employer assessments. Results become verified evidence in your Career Passport.' : 'Solve employer missions and contribute to open challenges. Every finished outcome strengthens your Career Passport.'}</p>
            <div className="mt-6 flex flex-wrap gap-3">
              <Link to="/missions" className={'product-focus rounded-xl px-4 py-2.5 text-sm font-extrabold ' + (!isTests ? 'bg-white text-[#12213a]' : 'border border-white/20 bg-white/10 text-white')}>Missions & challenges</Link>
      ...
      <section className="mt-8">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div><p className="text-xs font-extrabold uppercase tracking-[0.14em] text-[#155eef]">{isTests ? 'Assessment library' : 'Opportunity board'}</p><h2 className="mt-1 text-2xl font-extrabold tracking-[-0.04em] text-slate-950 dark:text-white">{isTests ? 'Employer skill tests' : 'Missions & community challenges'}</h2></div>
          <div className="flex flex-col gap-2 sm:flex-row">
            <label className="flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 shadow-sm"><Search className="h-4 w-4 text-slate-400" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search skills or employers" className="w-full bg-transparent text-sm outline-none sm:w-52" /></label>
