...
function Analytics() {
  const stats = [['Applications','347','+18%'],['Qualified','126','+12%'],['Interviews','48','+9%'],['Hires','14','+4%']];
  return <Shell eyebrow="Hiring intelligence" title="Hiring analysis" copy="Understand pipeline health, conversion, response performance, and demand by role.">
    ...
    <div className="mt-5 grid gap-5 lg:grid-cols-[1.5fr_1fr]"><section className="product-surface p-6"><h2 className="text-xl font-black">Applicant volume</h2><p className="text-sm text-slate-500">Eight-week qualified pipeline</p><div className="mt-8 flex h-64 items-end gap-3 border-b border-slate-200">{weekly.map((value,i) => <div key={i} className="flex flex-1 flex-col items-center gap-2"><strong className="text-xs">{value}</strong><span className="w-full rounded-t-lg bg-[#173b67]" style={{ height: `${value * 2}px` }} /><small className="text-slate-400">W{i+1}</small></div>)}</div></section><section className="product-surface p-6"><h2 className="text-xl font-black">Role conversion</h2>{[['Engineering',68],['Product & design',57],['Data',49],['Marketing',42]].map(([name, score]) => <div key={name} className="mt-6"><div className="flex justify-between text-sm font-bold"><span>{name}</span><span>{score}%</span></div><div className="mt-2 h-2 rounded-full bg-slate-100"><div className="h-full rounded-full bg-[#b7ff3c]" style={{ width: `${score}%` }} /></div></div>)}</section></div>
  </Shell>;
}
